...
interface ErrorMonitoringEvent {
  round: number
  timestamp: number
...
interface SessionData {
  ...
  workingMemory: {
    sequentialCapacity: number
    maxSpanReached: number
    averageSpan: number
    spanProgressionRate: number
  ...
  concentration: {
    sustainedConcentrationScore: number
    totalLapses: number
    averageLapseDuration: number
  ...
  detailedLogs: {
    ...
    processingSpeedMetrics: ProcessingSpeedMetric[]
    errorMonitoringEvents: ErrorMonitoringEvent[]
    concentrationLapses: ConcentrationLapse[]
  }
}
...
export default function NumberSequenceGame() {
  ...
  const [sessionData, setSessionData] = useState<SessionData>({
    ...
    totalCorrect: 0,
    workingMemory: {
      sequentialCapacity: 3,
      maxSpanReached: 3,
      averageSpan: 3,
      spanProgressionRate: 0,
      retentionAccuracy: 100
    },
    attention: {
    ...
    concentration: {
      sustainedConcentrationScore: 100,
      totalLapses: 0,
  ...
  const digitInputTimes = useRef<number[]>([])
  const lastConcentrationLapse = useRef(Date.now())
  ...
  const handleNumberClick = (num: number) => {
    ...
    const attentionEvent: AttentionEvent = {
      ...
      timestamp: now,
      digitPosition,
      reactionTime: timeSinceLastAction,
      sustained: timeSinceLastAction < 3000
    }
    ...
    setSessionData(prev => ({
      ...
      attention: {
        ...prev.attention,
        attentionLapses: prev.attention.attentionLapses + (timeSinceLastAction > 3000 ? 1 : 0)
      },
      detailedLogs: {
        ...prev.detailedLogs,
    ...
    digitInputTimes.current.push(timeSinceLastAction)
    if (digitInputTimes.current.length >= 3) {
      const recentSpeeds = digitInputTimes.current.slice(-5)
      const avgSpeed = recentSpeeds.reduce((a, b) => a + b, 0) / recentSpeeds.length
      const variance = recentSpeeds.reduce((sum, speed) => sum + Math.pow(speed - avgSpeed, 2), 0) / recentSpeeds.length
      
      const speedMetric: ProcessingSpeedMetric = {
        round,
        timestamp: now,
        averageInputSpeed: avgSpeed,
      ...
      setSessionData(prev => ({
        ...
        processingSpeed: {
          ...
          fastestResponse: Math.min(prev.processingSpeed.fastestResponse, timeSinceLastAction),
          slowestResponse: Math.max(prev.processingSpeed.slowestResponse, timeSinceLastAction)
        },
        detailedLogs: {
          ...prev.detailedLogs,
          processingSpeedMetrics: [...prev.detailedLogs.processingSpeedMetrics, speedMetric]
        }
      }))
    }
    
    if (timeSinceLastAction > hesitationThreshold) {
      const lapseDuration = timeSinceLastAction - hesitationThreshold
      const recoveryTime = now - lastConcentrationLapse.current
      
      const concentrationLapse: ConcentrationLapse = {
        round,
        timestamp: now,
        duration: lapseDuration,
        recoveryTime
      }
      ...
      setSessionData(prev => ({
        ...
        concentration: {
          ...prev.concentration,
          totalLapses: prev.concentration.totalLapses + 1
        },
        detailedLogs: {
          ...prev.detailedLogs,
          concentrationLapses: [...prev.detailedLogs.concentrationLapses, concentrationLapse]
        }
      }))
      ...
      lastConcentrationLapse.current = now
    }
    
    const newInput = [...userInput, num]
    setUserInput(newInput)
    
    const isError = num !== sequence[digitPosition]
    const errorMonitoringEvent: ErrorMonitoringEvent = {
      round,
      timestamp: now,
    ...
    if (isError) {
      ...
      setSessionData(prev => ({
        ...prev,
        errorMonitoring: {
          ...prev.errorMonitoring,
          totalErrors: prev.errorMonitoring.totalErrors + 1,
          substitutionErrors: prev.errorMonitoring.substitutionErrors + 1
        },
        detailedLogs: {
          ...prev.detailedLogs,
          errorMonitoringEvents: [...prev.detailedLogs.errorMonitoringEvents, errorMonitoringEvent]
        }
      }))
    }
  ...
  const submitAnswer = (finalInput: number[]) => {
    ...
    setSessionData(prev => {
      const allSpeeds = prev.detailedLogs.processingSpeedMetrics.map(m => m.averageInputSpeed)
      const avgSpeed = allSpeeds.length > 0 ? allSpeeds.reduce((a, b) => a + b, 0) / allSpeeds.length : inputSpeed
      const speedVariance = allSpeeds.length > 0 
        ? allSpeeds.reduce((sum, speed) => sum + Math.pow(speed - avgSpeed, 2), 0) / allSpeeds.length 
        : 0
      
      const allReactionTimes = prev.detailedLogs.attentionEvents.map(e => e.reactionTime)
      const avgReactionTime = allReactionTimes.length > 0 
        ? allReactionTimes.reduce((a, b) => a + b, 0) / allReactionTimes.length 
        : 0
      
      const concentrationLapses = prev.detailedLogs.concentrationLapses
      const avgLapseDuration = concentrationLapses.length > 0
        ? concentrationLapses.reduce((sum, l) => sum + l.duration, 0) / concentrationLapses.length
        : 0
      
      const totalAttempts = attempts.length + 1
      const sessionDuration = now - prev.sessionStart
      const fatigueIndex = concentrationLapses.length > 0
        ? (concentrationLapses.length / (sessionDuration / 60000)) * 10
        : 0
      
      return {
        ...prev,
        maxSequenceReached: Math.max(prev.maxSequenceReached, sequenceLength),
        workingMemory: {
          sequentialCapacity: sequenceLength,
          maxSpanReached: Math.max(prev.workingMemory.maxSpanReached, sequenceLength),
          averageSpan: ((prev.workingMemory.averageSpan * (totalAttempts - 1)) + sequenceLength) / totalAttempts,
          spanProgressionRate: (sequenceLength - 3) / Math.max(round, 1),
          retentionAccuracy: accuracy
        },
        attention: {
          ...prev.attention,
          averageReactionTime: avgReactionTime,
          sustainedAttentionScore: Math.max(0, 100 - (prev.attention.attentionLapses * 5)),
          attentionVariability: Math.sqrt(speedVariance),
          focusDuration: sessionDuration
        },
        processingSpeed: {
          ...prev.processingSpeed,
          averageInputSpeed: avgSpeed,
          speedConsistency: Math.max(0, 100 - (Math.sqrt(speedVariance) / Math.max(avgSpeed, 1)) * 100),
          speedImprovement: allSpeeds.length > 1 
            ? ((allSpeeds[0] - allSpeeds[allSpeeds.length - 1]) / allSpeeds[0]) * 100 
            : 0
        },
        errorMonitoring: {
          ...prev.errorMonitoring,
          omissionErrors: prev.errorMonitoring.omissionErrors + omissions,
          errorDetectionRate: prev.errorMonitoring.totalErrors > 0
            ? (prev.errorMonitoring.selfCorrectionAttempts / prev.errorMonitoring.totalErrors) * 100
            : 100,
          errorAwareness: isCorrectAnswer ? 100 : Math.max(0, 100 - ((omissions + wrongOrders) * 10))
        },
        concentration: {
          ...prev.concentration,
          sustainedConcentrationScore: Math.max(0, 100 - (prev.concentration.totalLapses * 3)),
          averageLapseDuration: avgLapseDuration,
          recoveryEfficiency: concentrationLapses.length > 1
            ? Math.max(0, 100 - (avgLapseDuration / 1000) * 5)
            : 100,
          fatigueIndex: Math.min(100, fatigueIndex)
        }
      }
    })
  ...
  const exportMetrics = () => {
    const metrics = {
      ...
      cognitiveProfile: {
        ...
        attention: sessionData.attention,
  ...
  const resetGame = () => {
    ...
    setErrors([])
    digitInputTimes.current = []
    lastConcentrationLapse.current = Date.now()
    setSessionData({
      sessionStart: Date.now(),
      maxSequenceReached: 3,
      totalRounds: 0,
    ...
    <main className="min-h-screen bg-gradient-to-br from-purple-100 via-pink-100 to-rose-100 text-gray-900 flex flex-col items-center justify-center p-6">
      <div className="text-center mb-8">
        ...
        <div className="flex gap-6 justify-center text-sm text-gray-600 flex-wrap">
          ...
          <span className="bg-blue-200 px-3 py-1 rounded-full">Lapses: {sessionData.concentration.totalLapses}</span>
        </div>
      ...
      {showMetrics && (
        <div className="bg-white/30 backdrop-blur-sm border border-white/40 rounded-2xl p-6 max-w-6xl w-full">
          <h3 className="font-bold text-purple-700 mb-4 text-center text-2xl">🧠 Cognitive Metrics</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
            <div className="bg-white/40 rounded-lg p-3">
              <h4 className="font-semibold text-purple-600 mb-2">🧮 Working Memory</h4>
              <div>Current Span: {sessionData.workingMemory.sequentialCapacity}</div>
              <div>Max Span: {sessionData.workingMemory.maxSpanReached}</div>
              <div>Avg Span: {sessionData.workingMemory.averageSpan.toFixed(1)}</div>
              <div>Retention: {sessionData.workingMemory.retentionAccuracy.toFixed(0)}%</div>
            </div>
            <div className="bg-white/40 rounded-lg p-3">
              <h4 className="font-semibold text-pink-600 mb-2">👁️ Attention</h4>
              <div>Score: {sessionData.attention.sustainedAttentionScore.toFixed(0)}%</div>
              <div>Lapses: {sessionData.attention.attentionLapses}</div>
              <div>Avg Reaction: {sessionData.attention.averageReactionTime.toFixed(0)}ms</div>
              <div>Variability: {sessionData.attention.attentionVariability.toFixed(0)}ms</div>
            </div>
            <div className="bg-white/40 rounded-lg p-3">
              <h4 className="font-semibold text-rose-600 mb-2">⚡ Processing Speed</h4>
              <div>Avg Speed: {sessionData.processingSpeed.averageInputSpeed.toFixed(0)}ms</div>
              <div>Consistency: {sessionData.processingSpeed.speedConsistency.toFixed(0)}%</div>
              <div>Fastest: {sessionData.processingSpeed.fastestResponse === Infinity ? 0 : sessionData.processingSpeed.fastestResponse.toFixed(0)}ms</div>
              <div>Slowest: {sessionData.processingSpeed.slowestResponse.toFixed(0)}ms</div>
            </div>
            <div className="bg-white/40 rounded-lg p-3">
              <h4 className="font-semibold text-indigo-600 mb-2">🎯 Error Monitoring</h4>
              <div>Total Errors: {sessionData.errorMonitoring.totalErrors}</div>
              <div>Omissions: {sessionData.errorMonitoring.omissionErrors}</div>
              <div>Substitutions: {sessionData.errorMonitoring.substitutionErrors}</div>
              <div>Awareness: {sessionData.errorMonitoring.errorAwareness.toFixed(0)}%</div>
            </div>
            <div className="bg-white/40 rounded-lg p-3">
              <h4 className="font-semibold text-blue-600 mb-2">🎯 Concentration</h4>
              <div>Score: {sessionData.concentration.sustainedConcentrationScore.toFixed(0)}%</div>
              <div>Lapses: {sessionData.concentration.totalLapses}</div>
              <div>Avg Lapse: {sessionData.concentration.averageLapseDuration.toFixed(0)}ms</div>
              <div>Fatigue: {sessionData.concentration.fatigueIndex.toFixed(1)}%</div>
            </div>
            <div className="bg-white/40 rounded-lg p-3">
          ...
          <div className="mt-4 p-4 bg-white/20 rounded-lg">
            <h4 className="font-semibold text-indigo-600 mb-2 text-center">⏱️ Timing Analysis</h4>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs text-center">
              ...
              <div>
                ...
                <div className="text-gray-600">Hesitations</div>
              </div>
              <div>
                <div className="font-bold text-lg text-pink-600">
                  {hesitations.length > 0 ? 
                    (hesitations.reduce((sum, h) => sum + h.duration, 0) / hesitations.length / 1000).toFixed(1) : 0}s
                </div>
              ...
              <div>
                <div className="font-bold text-lg text-rose-600">
                  {attempts.length > 0 ? 
                    (attempts.reduce((sum, a) => sum + a.inputSpeed, 0) / attempts.length / 1000).toFixed(1) : 0}s
                </div>
                <div className="text-gray-600">Avg Input Speed</div>
              </div>
            </div>
          </div>
        </div>
