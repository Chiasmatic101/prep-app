...
// PATCH E: Tiny multivariate importance (MVP)
export function featureImportances(X: number[][], y: number[], lambda = 0.5): number[] {
  // Very small ridge-ish fit to get relative importances.
  // X: rows of standardized lifestyle features per day
  // y: standardized domain score per day
  ...
  // Solve (naive Gauss-Jordan for small nFeat)
  // Augment [XTX | XTy]
  ...
  for (let col = 0; col < nFeat; col++) {
    ...
    if (Math.abs(A[piv][col]) < 1e-8) continue;
  ...
  // Normalize to sum of abs = 1 for display ordering
...
class IntegratedSyncScoreCalculator {
  ...
  // PATCH C1: Add explicit final score combiner
  private computeFinalSyncScore(
    baseSync: number,
    adjustments: EnhancedSyncResults['dynamicAdjustments'],
    trend: EnhancedSyncResults['trendAnalysis']
  ): number {
    const {
      sleepQualityBonus,
    ...
    const wAdj = 0.35;   // total influence of adjustments
    const wTrend = 0.15; // trend nudge
    const trendNudge = trend.weeklyTrend === 'improving'
      ? +5
    ...
    const adjSum =
      0.30 * sleepQualityBonus +
      0.25 * nutritionTimingBonus +
      0.25 * exerciseBonus +
      0.10 * challengeProgressBonus +
      0.10 * consistencyBonus;

    const combined =
      (1 - wAdj - wTrend) * baseSync +
      wAdj * (baseSync + adjSum) +
      wTrend * (baseSync + trendNudge);

    return Math.max(0, Math.min(100, Math.round(combined)));
  }

  // PATCH C2: Implement extractOptimalTimeFromInsight()
  private extractOptimalTimeFromInsight(insight: CorrelationInsight): number | null {
    const txt = `${insight.factor} ${insight.outcome}`.toLowerCase();

    // 1) Direct hour tokens: "at 15:00", "15–17h"
    const hhmm = txt.match(/\b([01]?\d|2[0-3]):([0-5]\d)\b/);
    if (hhmm) {
    ...
    const hourRange = txt.match(/\b([01]?\d|2[0-3])\s?[-–]\s?([01]?\d|2[0-3])\s?h?\b/);
    if (hourRange) {
      const h1 = parseInt(hourRange[1], 10);
      const h2 = parseInt(hourRange[2], 10);
      const mid = h1 <= h2 ? (h1 + h2) / 2 : ((h1 + (h2 + 24)) / 2) % 24;
      return mid;
    }

    // 2) Dayparts
    if (txt.includes('early morning')) return 7;
    if (txt.includes('morning')) return 9.5;
    if (txt.includes('late morning')) return 11;
    if (txt.includes('midday') || txt.includes('noon')) return 12.5;
    if (txt.includes('afternoon')) return 15;
    if (txt.includes('late afternoon')) return 16.5;
    if (txt.includes('evening')) return 19;
    if (txt.includes('late night') || txt.includes('night')) return 22;

    // 3) Phrases like "best between 13–16"
    const bestBetween = txt.match(/best (?:between|from)\s+([01]?\d|2[0-3])\s?[-–]\s?([01]?\d|2[0-3])/);
    if (bestBetween) {
      const a = parseInt(bestBetween[1], 10);
      const b = parseInt(bestBetween[2], 10);
      return (a + b) / 2;
    }
  ...
  private calculateLearningAcrophase(
    ...
    // Adjust based on lifestyle feedback insights
  ...
  async calculateIntegratedSync(
    ...
    // Calculate dynamic adjustments and trend analysis
    const adjustments = this.calculateDynamicAdjustments(
      lifestyleFeedback, 
      sleepMetrics, 
      challengeData
    );
    const trend = this.calculateTrendAnalysis(cognitiveData, lifestyleFeedback);

    // Use the new final score combiner
    const finalSyncScore = this.computeFinalSyncScore(baseSyncScore, adjustments, trend);
